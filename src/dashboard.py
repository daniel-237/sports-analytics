...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("Predict Match →"):
        ...
        if h_conceded < a_conceded:
            reasons.append(f" {home_team} have conceded fewer goals recently ({h_conceded:.1f} vs {a_conceded:.1f}/game)")
        else:
            reasons.append(f" {away_team} have a stronger defensive record recently ({a_conceded:.1f} vs {h_conceded:.1f}/game)")
        reasons.append(f" Home advantage is factored into all predictions")
        if abs(conf - 0.5) < 0.1:
            reasons.append(f" Low confidence — both teams have similar recent form")
        ...
        c3.metric(" Away Win",  f"{probs[0]:.0%}")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if home_wins/max(len(home_df),1) > away_wins/max(len(away_df),1) * 1.3:
        ...
        insight_card("", f"<b>{team}</b> are surprisingly strong away from home.")
    else:
        insight_card("", f"<b>{team}</b> perform consistently both home and away.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab3:
        ...
        if attack_diff < -0.2:
            weaknesses.append(f" **Weak attack** — scoring {avg_scored:.2f} goals/game vs league avg {league_avg_scored:.2f}")
        elif attack_diff > 0.2:
            strengths.append(f" **Strong attack** — scoring {avg_scored:.2f} goals/game vs league avg {league_avg_scored:.2f}")

        if defence_diff > 0.2:
            weaknesses.append(f" **Leaky defence** — conceding {avg_conceded:.2f} goals/game vs league avg {league_avg_conceded:.2f}")
        ...
        if fail_score > 0.25:
            weaknesses.append(f" **Struggling to score** — fails to score in {fail_score:.0%} of games")
        if clean_sheets < 0.2:
            weaknesses.append(f" **Defensive frailty** — keeps clean sheet in only {clean_sheets:.0%} of games")
        ...
        if weaknesses:
            ...
            if any("defence" in w.lower() or "defensive" in w.lower() for w in weaknesses):
                insight_card("", f"<b>{team_tw}</b> need defensive reinforcement. Consider a defender with high tackle/interception rates.")
                defenders = players[
    ...
    with tab4:
        ...
        insight_card("",  f"<b>{best_att['team']}</b> — best attack at <b>{best_att['attack']:.2f}</b> goals/game.")
        insight_card("", f"<b>{best_def['team']}</b> — best defence conceding only <b>{best_def['defence']:.2f}</b>/game.")
        st.markdown("<br>", unsafe_allow_html=True)
