...
# =============================================================================
    ...
    if st.button("Predict Match →"):
        ...
        c3.metric(" Away Win", f"{ordered.loc[2, 'Probability']:.0%}")
        ...
        if home_conceded <= away_conceded:
            reasons.append(f" {home_team} have conceded fewer goals recently ({home_conceded:.1f} vs {away_conceded:.1f} per game).")
        else:
            reasons.append(f" {away_team} have the stronger recent defensive record ({away_conceded:.1f} vs {home_conceded:.1f} per game).")

        if confidence < 0.45:
            reasons.append(" The model confidence is low, so this match should be treated as close.")
        else:
...
# =============================================================================
    ...
    if home_win_rate > away_win_rate * 1.3 and home_win_rate > 0:
        ...
        insight_card("", f"<b>{team}</b> perform fairly consistently home and away.")
...
# =============================================================================
    ...
    tab1, tab2, tab3, tab4 = st.tabs([" Player Scouting", " Hidden Gems", " Team Weaknesses", " Attack vs Defence"])
    ...
    with tab3:
        ...
        if any("defence" in item.lower() or "defensive" in item.lower() for item in weaknesses):
            insight_card("", f"<b>{team_tw}</b> need defensive reinforcement. Look for high tackles and interceptions per 90.")
